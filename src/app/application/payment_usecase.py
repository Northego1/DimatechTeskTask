...
class AccountRepositoryProtocol(Protocol):
    async def get_account(self, account_id: uuid.UUID, *,
            include_payments: bool = False) -> Account | None: ...

    async def add_account(self, account: Account) -> uuid.UUID | None: ...


class PaymentRepositoryProtocol(Protocol):
    async def create_payment(self, payment: Payment) -> uuid.UUID | None: ...
...
class PaymentUsecase:
    ...
    async def execute(self, payments_data: dto.PaymentData) -> None:
        async with self.uow.transaction() as repo:
            ...
            if not (account := await repo.account_repository.get_account(
